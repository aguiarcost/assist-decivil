...
<style>
    ...
    .stApp h1, .stApp h2, .stApp h3 { color: #ef6c00; }
</style>
""", unsafe_allow_html=True)

st.title("🧡 Felisberto, Assistente Administrativo ACSUTA")

# Função: carregar base
@st.cache_data
...
if os.path.exists(CAMINHO_HISTORICO):
    try:
        ...
        pass
perguntas_frequentes = sorted({p["pergunta"] for p in base_conhecimento}, key=lambda x: -frequencia.get(x, 0))

# Interface de pergunta
col1, col2 = st.columns(2)
with col1:
    pergunta_dropdown = st.selectbox("Escolha uma pergunta frequente:", [""] + perguntas_frequentes)
with col2:
...
if novo_json:
    try:
        ...
        if isinstance(novas_perguntas, list):
            ...
            st.success("✅ Base atualizada. Reinicie para ver as novas perguntas.")
        else:
...
with st.expander("➕ Adicionar nova pergunta manualmente"):
    ...
    if st.button("Guardar pergunta"):
        if nova_pergunta and nova_resposta:
            ...
            with open(CAMINHO_CONHECIMENTO, "w", encoding="utf-8") as f:
                json.dump(list(todas.values()), f, ensure_ascii=False, indent=2)
            st.success("✅ Pergunta adicionada. Reinicie para ver no menu.")
        else:
