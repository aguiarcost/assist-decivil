...
resposta = ""
if pergunta_final and pergunta_final != st.session_state.ultima_pergunta:
...
elif st.session_state.get("ultima_resposta"):
...
# Atualizar base de conhecimento via JSON
st.markdown("---")
...
if novo_json:
    try:
        ...
        if isinstance(novas_perguntas, list):
            ...
            st.success("✅ Base atualizada. Recorra a `gerar_embeddings.py` para atualizar os embeddings.")
        else:
...
with st.expander("➕ Adicionar nova pergunta manualmente"):
    ...
    if st.button("Guardar pergunta"):
        if nova_pergunta and nova_resposta:
            ...
            with open(CAMINHO_CONHECIMENTO, "w", encoding="utf-8") as f:
                json.dump(list(todas.values()), f, ensure_ascii=False, indent=2)
            st.success("✅ Pergunta adicionada. Recorra a `gerar_embeddings.py` para atualizar os embeddings.")
        else:
